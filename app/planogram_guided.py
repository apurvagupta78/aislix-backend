...
def _is_ambiguous_lays_flavor_ocr(text: str) -> bool:
    """Short OCR fragments (e.g. 'Cream', 'Tom', 'Tango') must not override row/color assignment."""
    ...
    standalone = {
        ...
        "eam & o", "eam & on", "tango", "tomato", "tom", "tomat", "magic", "masala",
    }
    ...
    if "tomato" in blob or "tango" in blob:
        return True
    if "magic" in blob and "masala" in blob:
        return False
    ambiguous = {
        "cream",
        "crear",
...
def assign_planogram_shelf_rows(
    ...
    for cluster_idx, row in enumerate(row_clusters):
        ...
        for rec, ocr in zip(row, row_ocrs):
            from app.snack_row_recovery import _is_weak_tomato_pack_text

            bag_color = _bag_color_family(source_image, rec) if source_image is not None else "unknown"
            row_bag_color = (
                max(row_color_votes, key=row_color_votes.get)
                if row_color_votes and _row_allows_lays_color_recovery(row)
                else None
            )
            if bag_color == "unknown" and row_bag_color:
                votes_total = sum(row_color_votes.values())
