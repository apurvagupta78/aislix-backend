    ...
    try:
        ...
        if planogram_items:
            has_expected_facings = any(
                item.get("expected_facings") not in (None, "") for item in planogram_items
            )
            has_placement_rules = any(str(item.get("shelf_position") or "").strip() for item in planogram_items)
